...
func (q *diskQueue) evictLocked(it backlogItem, reason, detail string) bool {
	if err := os.RemoveAll(it.path); err != nil {
		log.Errorf("failed to evict queued Cloudability data %s: %v", it.path, err)
		return false
	}
	if q.noUploader.Load() {
	...
	dropData(q.events, reason, 1, fmt.Sprintf("evicted %s %s (%d bytes, from %s): %s",
		kind, filepath.Base(filepath.Clean(it.path)), it.size, it.ts.UTC().Format(time.RFC3339), detail))
	return true
}

// makeRoom makes sure the scratch volume has need bytes free, evicting the oldest queued data,
// samples of clusterID and payloads alike, until it has (F-18). pressure reports whether space
